...
const SingleNFTTransfer = () => {
    ...
    return (
        <div className='single-nft-transfer-container'>
            ...
            {show && <NFTTransferDialog onClose={() => setShow(false)} tokenid={state.tokenID} addr={state.contractAddress} chainid={chainId} nftName={state.name} nftSymbol={state.symbol} uri={state.tokenUri} />}
        </div>
    )
}
